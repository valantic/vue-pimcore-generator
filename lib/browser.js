...
{% else %}
    ${beautify(snippet.outerHTML)}
{% endif %}
`);
      await writeFile(getSnippetTwigPath(snippet.name, 'edit'), beautify(snippet.outerHTML));
    })
    .each(async(snippet) => {
      const predefinedDocument = {
        ...
        action: 'default',
        template: getSnippetTwigPath(snippet.name).replace(`${twigBasepath}/`, ''),
        type: 'snippet',
        priority: 0,
