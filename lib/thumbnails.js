...
function run() {
  ...
  const thumbnailsString = thumbnailsUnique.join(' ');

  log('note', 'Ensuring the following definitions exist: %s', thumbnailsUnique.join(', '));

  // TODO: creates an non-named entry in the Pimcore image definitions.
  if (thumbnailsString) {
    execSync(`php ${phpHelpersPath('thumbnail-ensure.php')} ${pimcoreBasepath} ${thumbnailsString}`);
  }
