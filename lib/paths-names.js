...
/**
 * The root path of snippet Twig template files.
 *
 * @type {string}
 */
const snippetTwigBasepath = _.defaultTo(
  definitions.snippetTwigBasepath,
  base => path.join(base, 'app', 'Resources', 'views', 'snippets')
)(pimcoreBasepath);
...
/**
 * Returns the full path to an snippet Twig template.
 *
 * @param {String} name The name of the areabrick
 * @param {String} suffix Whether the template is for `view` or `edit`
 * @returns {String}
 */
const getSnippetTwigPath = (name, suffix = 'view') => path.join(
  snippetTwigBasepath,
  _.kebabCase(name), `${suffix}.html.twig`
);
