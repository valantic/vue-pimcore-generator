...
async function findBricks(page) {
  ...
  return await page.evaluate(() => {
    ...
    nodes.forEach((node) => {
      ...
      const brick = { name: node.dataset.pimcoreAreabrick, html: node.innerHTML };

      delete node.dataset.pimcoreArebrick;
      bricks.push(brick);
    });
...
async function findSnippets(page) {
  ...
  return await page.evaluate(() => {
    const snippets = [];
    const nodes = document.querySelectorAll('[data-pimcore-is-snippet]');
    ...
    nodes.forEach((node) => {
      ...
      if (node.dataset.pimcoreDocument) {
        const snippet = { name: node.dataset.pimcoreDocument.substr(1), html: node.innerHTML };
...
async function disableNavigation(page) {
  await page.evaluate(() => {
    const nodes = document.querySelectorAll('[data-pimcore-disable-editmode]');

    nodes.forEach((node) => {
      node.querySelectorAll('a').forEach((disablable) => {
        disablable.classList.add('disabled-in-editmode');
      });
      delete node.dataset.pimcoreDisableEditmode;
    });
  });
}
