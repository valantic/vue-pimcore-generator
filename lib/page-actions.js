  ...
  return await page.evaluate(() => {
    ...
    nodes.forEach((node) => {
      ...
      if (!name) {
        ...
        log('error', 'No areabrick name was defined on "%s".', html);
  ...
  return await page.evaluate(() => {
    ...
    nodes.forEach((node) => {
      ...
      if (!node.dataset.pimcoreSnippet) {
        ...
        log('error', 'No snippet name was defined on "%s".', html);
        throw new Error(`No snippet name was defined on '${html}'.`);
      }
      const snippet = { name: node.dataset.pimcoreSnippet, html: node.outerHTML };

      snippets.push(snippet);

      delete node.dataset.pimcoreSnippet;
    });
